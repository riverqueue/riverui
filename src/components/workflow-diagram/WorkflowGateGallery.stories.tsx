...
const GateCanvas = ({ children }: { children: ReactNode }) => (
  <div className="relative" style={{ height: BOX_HEIGHT, width: BOX_WIDTH }}>
    <span
      aria-hidden
      className={clsx(handleClass, "absolute top-0 left-0 z-10")}
    />
    <svg
    ...
    <span
      aria-hidden
      className={clsx(handleClass, "absolute top-0 right-0 z-10")}
    />
  </div>
);
    ...
    <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          {variant.current ? (
            <span className="rounded-full bg-slate-100 px-1.5 py-0.5 text-[9px] font-medium tracking-wide text-slate-600 uppercase dark:bg-slate-700 dark:text-slate-300">
              Current
            </span>
          ) : null}
        </div>
        <span
          className={clsx(
            "rounded-full px-2 py-0.5 text-[10px] font-medium tracking-wide uppercase transition-colors duration-300",
            blocking
