...
const longCelExpressionsWait: WorkflowTaskWait = {
  exprCel: "approval_payload_ready && draft_quality_gate",
  inputs: {
  ...
  phase: "waiting",
  startedAt: sub(now, { minutes: 8 }),
  summary: "Waiting for approval payload and draft quality checks.",
  terms: [
    {
      exprCel: `payload.approved == true && payload.reviewer != "" && payload.review_score >= 90 && payload.source in ["console", "webhook"]`,
      kind: "signal",
      label: "Approval payload ready",
      name: "approval_payload_ready",
      result: { matchedCount: 0, requiredCount: 1, satisfied: false },
      signalKey: "approval.received",
    },
    {
      exprCel: `deps["verify_draft"].output.quality.score >= 0.95 && deps["verify_draft"].output.policy.status == "clear" && deps["verify_draft"].output.needs_human_review == false`,
      kind: "generic",
      label: "Draft passed quality gate",
...
export const LongCelExpressions: Story = {
  args: {
    loadTaskSignals: manySignalsLoader,
    taskName: "await/review",
    wait: longCelExpressionsWait,
    workflowID: "wf-story",
  },
